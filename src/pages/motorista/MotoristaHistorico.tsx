...
import { Navigation, MapPin, Package, Clock, User, ChevronDown, ChevronUp } from "lucide-react";
...
export default function MotoristaHistorico() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      // Get motorista id
      const motRes = await fetch(`${baseUrl}/rest/v1/motoristas?select=id&user_id=eq.${user.id}&limit=1`, { headers });
      const motData = await motRes.json();
      if (!motData?.[0]) { setLoading(false); return; }

      const motoristaId = motData[0].id;

      const res = await fetch(
        `${baseUrl}/rest/v1/corridas?select=id,origem_texto,destino_texto,distancia_km,duracao_min,preco_total_estimado,preco_itens,preco_km,com_ajudante,preco_ajudante,status,created_at,taxa_creditos_debitada,clientes(nome,foto_url)&motorista_id=eq.${motoristaId}&order=created_at.desc`,
        { headers }
      );
      ...
      setCorridas(
        corridasData.map((c: any) => ({
          ...
          taxa_creditos_debitada: c.taxa_creditos_debitada,
          cliente_nome: c.clientes?.nome || null,
          cliente_foto: c.clientes?.foto_url ? `${storageUrl}/${c.clientes.foto_url}` : null,
          itens: itensMap[c.id] || [],
        }))
      );
      setLoading(false);
        ...
        return (
          <Card key={c.id} className="shadow-sm">
            <CardContent className="p-4 space-y-2">
              ...
              <Button variant="ghost" size="sm" className="w-full" onClick={() => setExpandedId(expanded ? null : c.id)}>
                ...
                <div className="space-y-3 pt-2 border-t">
                  {c.cliente_nome && (
                    <div className="flex items-center gap-3">
                      {c.cliente_foto ? (
                        <img src={c.cliente_foto} alt="Cliente" className="h-10 w-10 rounded-full object-cover" />
                      ) : (
                  ...
                  <div className="space-y-1 text-sm">
                    ...
                    {c.taxa_creditos_debitada != null && (
                      <div className="flex justify-between text-destructive">
                        <span>Taxa debitada</span>
                        <span>-R$ {Number(c.taxa_creditos_debitada).toFixed(2)}</span>
                      </div>
                    )}
